	...
	export default {
		...
		watch: {
			...
			value: function(value) {
				if (value !== this.wangEditor.txt.html()) {
					this.isClear = false;
					this.wangEditor.txt.html(this
					.value); //value为编辑框输入的内容，这里我监听了一下值，当父组件调用得时候，如果给value赋值了，子组件将会显示父组件赋给的值
				}
			}
		},
		mounted() {
		...
		methods: {
			initEditor() {
				...
				this.wangEditor.customConfig = this.wangEditor.customConfig ? this.wangEditor.customConfig : this
					.wangEditor.config
